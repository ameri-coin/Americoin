...
#if defined(NDEBUG)
# error "Americoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x040cf56ce4252ab5637322ef232902af6677ec05d6841511d84366dbce658461");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Americoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Americoin Signed Message:\n";
...
{
    // Americoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Americoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Americoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1485057010, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "Americas shining moment a stump speech";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 1 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04e5b8419f6b23540d0d1c65abd377f57974c5f451c1e90d0e8593ea2cee4f3b7c1727a96582101eb4efcac54db65a2bf825016a19455b959f5bd7b1e925d36fd4") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1485057010;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1213284;
        ...
        assert(block.hashMerkleRoot == uint256("0xdf25097190b92e4e72781e74d4c063d1c42244b8424d307994f04229d0241bd7"));
...
unsigned char pchMessageStart[4] = { 0xf6, 0xc4, 0xb0, 0xc4 }; // Americoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AmericoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AmericoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AmericoinMiner(CWallet *pwallet)
{
    printf("AmericoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("americoin-miner");
    ...
    try { loop {
        ...
        printf("Running AmericoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("AmericoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AmericoinMiner, pwallet));
}
